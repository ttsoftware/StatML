...
class Gauss(object):
    ...
    def draw_gauss_multi(mean=None, covariance=None, likelihood_mean_function=None, sample=None, label='', fig=None, show=False, color=None):
        ...
        if mean is not None:
            plt.plot(mean.item(0), mean.item(1), '^', color=np.random.random(3), label=label + ' distribution dimension_means: ' + str((mean.item(0), mean.item(1))))
        ...
        if likelihood_mean_function is not None:
            ...
            plt.plot(sample_mean.item(0), sample_mean.item(1), 's', color=color, label=label + ' dimension_means: ' + str((sample_mean.item(0), sample_mean.item(1))))
            plt.plot(
                [mean.item(0), sample_mean.item(0)],
                [mean.item(1), sample_mean.item(1)],
                '-',
                color=color,
                label=label + " dimension_means deviation: " + str(np.linalg.norm((mean-sample_mean)))
            )
